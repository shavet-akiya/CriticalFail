...
export default function NavBar({ found = true }: NavBarProps) {
    ...
    if (!found) {
        // Blank
        return (
            <div className="fixed top-0 left-0 right-0 z-50 navbar bg-[#0B1215] pl-8 select-none">
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0B1215] flex items-center justify-between px-4 py-3 select-none">
            <div className="flex-1 font-metal-mania">
                <Link href="/" className="text-3xl red-colour">
                    Dungeon Scribe
                </Link>
            ...
            {menuOpen && (
                <ul className="absolute top-[72px] left-0 right-0 bg-[#0B1215] flex flex-col items-center space-y-4 py-6 border-t border-gray-700 lg:hidden">
