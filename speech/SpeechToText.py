...
class SpeechToText:
        ...
        try:
            ...
            try:
                ...
                if duration_str and duration_str != "N/A":
                    ...
                    print(
                        f"  → Audio duration: Unknown (will process without chunking)"
                    )
            ...
            result = self.whisper_model.transcribe(
                audio_file_path, batch_size=8, language="en"
            )
    ...
    def _transcribe_in_chunks(self, audio_file_path, total_duration):
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                print(
                    f"  → Chunk {i+1}/{num_chunks}: {start_time/60:.1f}-{(start_time+chunk_duration)/60:.1f} min"
                )
                ...
                print(
                    f"     ✓ Chunk saved: {os.path.getsize(chunk_file) / 1024 / 1024:.2f} MB"
                )

                # Transcribe chunk
                result = self.whisper_model.transcribe(
                    chunk_file, batch_size=4, language="en"  # Smaller batch for chunks
                )
    ...
    def _perform_diarization(self, audio_path, segments):
        ...
        try:
            ...
            if len(embeddings) < 2:
                print(
                    "    ⚠️  Not enough segments for clustering, assigning all to SPEAKER_00"
                )
