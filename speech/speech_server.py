...
def initialize_stt():
    ...
    if initialization_attempted:
        status = (
            "already loaded"
            if speech_to_text and speech_to_text.models_loaded
...
async def root():
    ...
    return {
        ...
        "status": (
            "ready"
            if is_ready
            else "initializing" if not initialization_attempted else "failed"
        ),
...
async def process_audio(
    ...
    if not speech_to_text:
        ...
        if not initialize_stt():
            return JSONResponse(
                status_code=503,
                content={
                    "error": "Speech service failed to initialize",
                    "success": False,
                },
            )
    ...
    try:
        ...
        thread = threading.Thread(
            target=process_audio_job, args=(job_id, str(saved_path))
        )
        thread.daemon = True
        thread.start()
...
async def get_job_status(job_id: str):
    ...
    if job_id not in jobs:
        return JSONResponse(
            status_code=404, content={"error": "Job not found", "success": False}
        )
